...
function modelEfficiency(actual, model) {
	return model > 0 ? `${(100 * actual / model).toFixed(1)}%` : "n/a";
}

function phaseMisses(counters = {}) {
	return ["H", "W1", "G", "W2"].reduce((sum, phase) => sum + (Number(counters[phase]) || 0), 0);
}

function renderNextTarget(ns, prep, prepRam, now) {
	const row = (label, value) => dashboardRow(ns, label, value);
	dashboardSection(ns, "Next target");
	if (!prep?.target) {
		row("Status", `${prep?.status || "DISABLED"}${prep?.reason ? ` | ${prep.reason}` : ""}`);
		return;
	}
	const waveEta = prep.active ? ` | wave ETA ${dashboardTime(Math.max(0, prep.active.finishAt - now))}` : "";
	row("Target", `${prep.target} | ${prep.status || "WAITING"}${waveEta}`);
	if (prep.health) row("Health",
		`money ${(100 * prep.health.money / Math.max(1, prep.health.max)).toFixed(1)}%` +
		` | security +${Math.max(0, prep.health.sec - prep.health.min).toFixed(3)}`);
	if (prep.candidate) row("Potential",
		`${cash(prep.candidate.potential)}/s ${prep.candidate.upperBound ? "UPPER BOUND" : "estimate"}`);
	if (prepRam > 0 || prep.preemptions || prep.failures) row("Prep",
		`${formatRam(prepRam)} RAM | ${Number(prep.preemptions) || 0} preemptions | ${Number(prep.failures) || 0} failures`);
	if (prep.error || prep.reason) row("Progress", prep.error || prep.reason);
}

function renderFleetSummary(ns, usedRam, totalRam, hostCount, cloudState) {
	const row = (label, value) => dashboardRow(ns, label, value);
	dashboardSection(ns, "Fleet");
	const maxed = cloudState.nextAction === "fleet maxed" ? " MAXED" : "";
	row("Fleet", `${formatRam(usedRam)} / ${formatRam(totalRam)} used` +
		` | ${hostCount} hosts | cloud ${cloudState.count}/${cloudState.limit}${maxed}`);
...
function renderDashboard(
	...
	row("Mode", `${cfg.requestedTarget === "auto" ? "AUTO" : "LOCKED"} | ${hackStatus}`);
	if (drain || recovery) row("Reason", (drain || recovery).reason);

	dashboardSection(ns, "Income");
	row("Income", `${cash(income60)}/s actual | ${cash(p.expected)}/s model | ${modelEfficiency(income60, p.expected)}`);
	row("Throughput", `${batch60.toFixed(3)}/${p.batchRate.toFixed(3)} batches/s actual/model` +
		` | ${cash(stats.money)} total | ${stats.profitable} paid`);

	dashboardSection(ns, "Current target");
	row(target, `money ${(100 * money / Math.max(1, maxMoney)).toFixed(1)}% | security +${Math.max(0, sec - minSec).toFixed(3)}`);
	row("Workload", `${running.size} running | ${queue.length} queued | ${formatRam(workerRam)} JIT RAM`);
	row("Scheduler", `${phaseMisses(pipe.misses)} misses | ${pipe.softRecoveries} local | ${pipe.recoveries} fallback | ${stats.restarts} restarts`);
	if (phaseMisses(pipe.misses) || pipe.softRecoveries || pipe.recoveries || stats.restarts || pipe.driftMax > cfg.gap) {
		row("Timing", `drift max ${pipe.driftMax.toFixed(2)}ms | spacing ${Number.isFinite(pipe.minSpacing) ? `${pipe.minSpacing.toFixed(1)}ms` : "n/a"}`);
	}

	const prep = cfg.backgroundPrep;
	renderNextTarget(ns, prep, prepRam, now);
	renderFleetSummary(ns, usedRam, totalRam, network.hosts.length, cloudState);
	...
	if (cfg.dashboardDetails) {
		...
		row("Target mode", cfg.requestedTarget === "auto" ? "AUTO" : "LOCKED");
		...
		row("Background", `${prep?.target || "none"} | ${prep?.status || "DISABLED"}`);
...
function renderSchedulerDashboard(ns, pool) {
	...
	if (snapshot.mode === "running") {
		...
		if (p.admissionReason || p.stats.allocationFails) dashboardRow(ns, "Pressure",
			`${p.stats.allocationFails} RAM failures${p.admissionReason ? ` | ${p.admissionReason}` : ""}`);
		if (pool.cfg.maxTargets > 1) dashboardRow(ns, "Slots", `1/${pool.cfg.maxTargets} | ${pool.note}`);
		if (pool.cfg.dashboardDetails && p.admissionSkips) dashboardRow(ns, "Budget skips", `${p.admissionSkips} load-limited slots`);
		if (pool.cfg.dashboardDetails && p.idleRetunes) dashboardRow(ns, "Idle replans", p.idleRetunes);
		if (!pool.cfg.dashboardDetails) ns.print("  More diagnostics: --dashboard-details true");
		return;
	}
	const row = (label, value) => dashboardRow(ns, label, value);
	const totalAllocationFails = rows.reduce((sum, p) => sum + p.allocationFails, 0);
	const totalAdmissionSkips = rows.reduce((sum, p) => sum + p.admissionSkips, 0);
	ns.clearLog();
	ns.print(`JIT DAEMON :: MULTI :: hacking ${ns.getHackingLevel()}`);

	dashboardSection(ns, "Overview");
	row("Targets", `${rows.length}/${pool.cfg.maxTargets} | priority ${pool.anchor}`);
	row("Income", `${cash(snapshot.income60)}/s actual | ${cash(snapshot.model)}/s model | ${modelEfficiency(snapshot.income60, snapshot.model)}`);
	row("Run total", `${cash(snapshot.earned)} across all target epochs`);
	if (pool.note) row("Next action", pool.note);

	renderNextTarget(ns, background, backgroundRam, now);

	dashboardSection(ns, "Pipelines");
	for (const p of rows) {
		row(p.target, `${p.mode} | ${p.role} | ${cash(p.income60)}/s actual | ${cash(p.model)}/s model | ${modelEfficiency(p.income60, p.model)}`);
		if (p.mode === "WARMUP") row("First hack", `ETA ${dashboardTime(p.eta)}`);
		row("Health", `money ${(100 * p.money / Math.max(1, p.maxMoney)).toFixed(1)}% | security +${Math.max(0, p.security - p.minSecurity).toFixed(3)}` +
			` | ${p.running} running | ${p.queued} queued`);
		const misses = phaseMisses(p.misses);
		if (misses || p.local || p.fallback || p.restarts) row("Scheduler",
			`${misses} misses | ${p.local} local | ${p.fallback} fallback | ${p.restarts} rebuilds`);
		if (["DRAINING", "RECOVERING", "RETIRING", "PREPARING", "TUNING", "IDLE"].includes(p.mode)) row("Reason", p.note);
		if (p.admissionReason || p.allocationFails) row("Pressure",
			`${p.allocationFails} RAM failures${p.admissionReason ? ` | ${p.admissionReason}` : ""}`);
		if (pool.cfg.dashboardDetails) {
			row("Plan", `${p.batchRate.toFixed(3)}/${p.modelBatchRate.toFixed(3)} batches/s actual/model | ${formatRam(p.workerRam)} JIT RAM`);
			...
			row("Misses", dashboardCounters(p.misses));
			row("Budget skips", `${p.admissionSkips} load-limited slots`);
			row("Target earned", `${cash(p.earned)} | ${p.paid} paid batches`);
			if (p.idleRetunes) row("Idle replans", p.idleRetunes);
		}
	}

	renderFleetSummary(ns, usedRam, totalRam, pool.network.hosts.length, pool.cloudState);
	row("Scheduler", `loop max ${pool.lagMax.toFixed(1)}ms | ${totalAllocationFails} RAM failures` +
		(pool.cfg.dashboardDetails ? ` | ${totalAdmissionSkips} budget skips` : ""));
