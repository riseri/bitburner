/** One-shot server migration operation. @param {NS} ns */
export async function main(ns) {
	const cfg = config(ns.args[0]), target = String(ns.args[1] || ""), from = ns.getHostname();
	try { const result = await ns.dnet.induceServerMigration(target); if (result.success) emit(ns, cfg, { kind: "migration", host: target, from }); }
	catch (error) { emit(ns, cfg, { kind: "error", host: target || from, error: `migration: ${String(error?.message ?? error)}` }); }
}
